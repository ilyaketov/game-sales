...
def run_sales_all(pipe: Pipeline, cat: pd.DataFrame, raws: dict, mappings: dict,
    ...
    for ch in SALES_INDIV:
        ...
        results[ch] = run_channel_m(
            ...
            ostatok_nach=_carry(ch), sobytiya=events,
    ...
    if raws.get("Plati") and raws.get("GGSel"):
        out = run_combined_m(
            ...
            cat, ostatok_nach=_carry("Plati+GGSel"), sobytiya=events,
...
def run_everything(pipe: Pipeline, cat: pd.DataFrame, raws: dict, mappings: dict,
    ...
    return {"purchases": purchases,
            ...
            "sales": results,
            "reconcile": rec,
            "review": review_consolidated(results)}
...
def review_consolidated(sales_results: dict) -> pd.DataFrame:
    """Единый лист ручной сверки по всем каналам продаж.

    Вход: {канал: результат run_channel_m/run_combined_m} (у каждого есть
    `review` и `new_listings`). Возврат отсортирован: действенные категории
    сверху, внутри — по убыванию нераспределённого спроса.
    """
    frames = []
