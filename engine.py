...
def _apply_catalog(df: pd.DataFrame, catalog: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    for _, r in out.iterrows():
        ...
        cid, cname, st = _lookup(r["Name"], exact, norm, billing_id, id_idx)
        if cid is not None:
            new_ids.append(cid)
...
def _apply_catalog_b2b(df: pd.DataFrame, catalog: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    for _, r in out.iterrows():
        billing_id = r["ID"] if pd.notna(r["ID"]) else None
        cid, cname, st = _lookup(r["Наименование Игры"], exact, norm, billing_id, id_idx)
        if cid is not None:
            new_ids.append(cid)
...
def build_b2b(df: pd.DataFrame,
              ...
              product_catalog: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    ...
    или 'Rights_holder'. Используется для правила (CIS) IGM → FZE при KRM.
    """
