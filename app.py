...
st.set_page_config(page_title="GameSalesFlow", page_icon="🎮", layout="wide")

try:
    _GH = storage.cfg_from_secrets(st.secrets)
except Exception:
    _GH = None
...
def _cached_pipe(r1: str, r2: str, genba: str):
    """Загруженный Pipeline кэшируется по хэш-путям биллинга: пока файлы те же,
    повторные взаимодействия не перезагружают ~50с биллинг."""
    return orch.load_pipeline(r1, r2, genba)
...
def _load_events(path: str):
    """События: {канал: df} если есть колонка Канал, иначе общий df [ID, События]."""
    if path is None:
        return None
    df = pd.read_csv(path) if path.endswith(".csv") else pd.read_excel(path, engine="calamine")
    df = df[pd.to_numeric(df["ID"], errors="coerce").notna()].copy()
    df["ID"] = df["ID"].astype("Int64")
    col = "События" if "События" in df.columns else df.columns[-1]
    df["События"] = pd.to_numeric(df[col], errors="coerce").fillna(0).astype(int)
    if "Канал" in df.columns:
        return {ch: g[["ID", "События"]].reset_index(drop=True)
                for ch, g in df.groupby("Канал")}
    return df[["ID", "События"]]
...
# ─────────────── Шапка ───────────────
st.title("GameSalesFlow")
st.caption("Автоматизация отчётности по игровым ключам для QuickBooks: закуп, "
           "продажи, движение склада и сверка — за один проход.")

with st.expander("Справка — как пользоваться"):
    st.markdown(
        "**Что делает приложение.** По биллингу и выгрузкам маркетплейсов строит "
        "отчёт по закупу (9 площадок), продажам и движению ключей (6 каналов), "
        "сверяет их между собой и готовит данные для QuickBooks.\n\n"
        "**Шаг 1. Загрузка.** Перетащите все файлы в одно поле — роль каждого "
        "определяется автоматически по колонкам.\n"
        "- *Обязательно (биллинг):* R1 «Универсальный отчёт», R2 «UniversalReport», genbaFile.\n"
        "- *Для продаж:* выгрузки кабинетов Eneba, Kinguin, Driffle, G2A, Plati, GGSel.\n"
        "- *Опционально:* «Остаток_нач» (перенос прошлого месяца) и «События» "
        "(перемещения/возвраты/корректировки склада).\n\n"
        "**Шаг 2. Режим.** «Прогнать всё» — закуп и продажи; «Только закуп» — "
        "нужен лишь биллинг; «Только продажи» — выбранные каналы.\n\n"
        "**Шаг 3. Проверка (бейджи над результатом).**\n"
        "- *Гейт Δ* — расхождение между «Загружено» в движении и закупом зоны "
        "(норма — 0).\n"
        "- *Закуп: неклассиф.* — ключи от нераспознанных поставщиков (0 — норма; "
        "иначе строки подсвечены в файле).\n"
        "- *Склад* — закрыт ли «Остаток_конец» по Событиям.\n"
        "- *Ручная сверка* — сколько позиций требуют ручного решения.\n\n"
        "**Шаг 4. Результат.** Аннотированный `.xlsx`: лист «Легенда и проблемы», "
        "гейт, закуп, продажи и движение по каналам, ручная сверка. Проблемные "
        "ячейки подсвечены цветом и снабжены комментариями. Отдельно — файл "
        "переноса остатков для следующего месяца.\n\n"
        "**Самообучение.** Подтверждённые сопоставления листингов сохраняются "
        "(при настроенном GitHub — коммитятся в репозиторий) и подхватываются в "
        "следующем месяце. Столбец «Регионы-кандидаты» в ручной сверке подсказывает "
        "варианты региона.\n\n"
        "**Пределы (не ошибки).** Регион ключа не выводится из выгрузок (складской "
        "факт) — часть позиций всегда уходит в ручную сверку. «События» нельзя "
        "вычислить из биллинга: это внешний учётный лог, подаётся отдельным файлом.")

st.divider()
st.subheader("Шаг 1 · Загрузка файлов")
st.caption("Перетащите все файлы разом: биллинг (R1 / R2 / genbaFile) и выгрузки "
           "кабинетов; при необходимости — «Остаток_нач» и «События». Роль каждого "
           "файла определяется автоматически.")
up = st.file_uploader("Файлы месяца", type=["xlsx", "csv"],
                      accept_multiple_files=True, key="all_files",
                      label_visibility="collapsed")

c1, c2 = st.columns([2, 3])
with c1:
    report_date = st.date_input("Дата отчёта (последний день месяца)",
                                value=pd.Timestamp("2026-05-31"))
with c2:
    with st.expander("Дополнительно — каталог и хранилище (GitHub)"):
        cat_up = st.file_uploader(
            "Свой catalog_master.csv (по умолчанию — встроенный)",
            type=["csv"], key="cat")
        if storage.available(_GH):
            ok, msg = storage.check(_GH)
            (st.success if ok else st.warning)(f"GitHub: {msg}")
        else:
            st.caption("GitHub не настроен — маппинги берутся локально из mappings/. "
                       "Для автосохранения добавьте секцию [github] (token, repo) "
                       "в Secrets (см. README).")

if not up:
    st.info("Загрузите файлы, чтобы начать: обязателен биллинг (R1 / R2 / genbaFile), "
            "для продаж — выгрузки кабинетов.")
    st.stop()
    ...
    with st.spinner("Распознаём файлы…"):
        for f in up:
            ...
            if k is None:
                ...
                conflicts.append((k, det[k]["name"], f.name))   # дубль типа
...
conflicts = st.session_state.get("_conflicts", [])
...
for i, k in enumerate(order):
    with grid[i % 3]:
        if k in det:
            ...
            st.markdown(f"— {KIND_LABEL[k]} · _не загружен_")
for k in ("carry", "events"):
    if k in det:
        with grid[(order.index(k)) % 3]:
            st.info(f"✓ {KIND_LABEL[k]} · опционально\n\n`{det[k]['name']}`")
if unknown:
    st.warning("Не распознаны и пропущены: " + ", ".join(unknown)
               + ". Проверьте, что это нужные выгрузки.")
if conflicts:
    for k, old, new in conflicts:
        st.warning(f"Два файла определились как «{KIND_LABEL[k]}»: `{old}` и `{new}`. "
                   f"Использован последний (`{new}`) — уберите лишний, если это ошибка.")
...
st.divider()
st.subheader("Шаг 2 · Режим и запуск")
mode = st.radio("Что прогнать", ["Прогнать всё", "Только закуп", "Только продажи"],
                horizontal=True,
                help="«Прогнать всё» — закуп и продажи; «Только закуп» — нужен лишь "
                     "биллинг; «Только продажи» — выбранные каналы.")
...
if mode == "Только закуп":
    ...
    sel_units = st.multiselect("Каналы продаж", SALES_UNITS, default=avail_units,
                               help="Plati и GGSel считаются единым юнитом "
                                    "(нужны оба файла).")
...
if run:
    ...
    events = _load_events(det["events"]["path"]) if "events" in det else None
    rd = pd.Timestamp(report_date)

    with st.spinner("Загрузка биллинга (~50с; кэшируется до смены файлов)…"):
        pipe = _cached_pipe(det["r1"]["path"], det["r2"]["path"], det["genba"]["path"])
    ...
    res = {"mode": mode, "errors": [], "events_fed": events is not None}
    if mode == "Только закуп":
        ...
        with st.spinner("Прогон продаж…"):
            sales = orch.run_sales_all(pipe, cat, raws, mappings, carry=carry_in,
                                       ...
                                       extra_demand=bundle_demand, errors=res["errors"])
        ...
        res["review"] = orch.review_consolidated(sales, catalog=cat)
...
if res:
    ...
    st.caption(f"Режим: **{res.get('mode', 'Прогнать всё')}** · период "
               f"{rdate:%m.%Y}. Показатели ниже — быстрая проверка качества прогона.")

    # Ошибки каналов (изоляция сбоев)
    for ch, msg in res.get("errors", []):
        st.warning(f"Канал «{ch}» пропущен из-за ошибки: {msg}")
    ...
    if "reconcile" in res:
        rec0 = res["reconcile"]
        bad0 = rec0[(rec0["Юнит"] != "—") & (rec0["Δ"] != 0)]
        b[0].metric("Гейт Δ", "0 ✅" if len(bad0) == 0 else f"{len(bad0)} ❌",
                    help="Загружено движения = закуп зоны")
    # закуп: неклассиф. поставщики
    if "purchases" in res:
        unc = sum(int(v["unclassified"]["Количество"].sum())
                  for v in res["purchases"].values() if len(v["unclassified"]))
        b[1].metric("Закуп: неклассиф.", "0 ✅" if unc == 0 else f"{unc} ⚠",
                    help="Ключи от нераспознанных поставщиков (возвращены в свод, подсвечены)")
    # склад / события
    if results:
        b[2].metric("Склад", "закрыт Событиями ✅" if res.get("events_fed")
                    else "без Событий ○",
                    help="С Событиями Остаток_конец сходится с учётом перемещений")
    # ручная сверка
    if "review" in res:
        b[3].metric("Ручная сверка", f"{len(res['review'])}",
                    help="Позиции для ручной проверки (регион/листинг)")
    ...
    if results and len(new_all):
        st.caption("Укажите catID (предзаполнен подсказкой из каталога) и нажмите "
                   "«Сохранить маппинги». При настроенном GitHub они коммитятся в "
                   "репозиторий и подхватятся в следующем месяце; иначе — скачайте zip "
                   "и положите в mappings/.")
        ...
        if st.button("Сохранить маппинги"):
            ...
            if not updated:
                ...
                if storage.available(_GH):
                    saved = storage.save_many(_GH, updated)
                    if saved:
                        st.success("Закоммичено в GitHub (" + storage.repo_label(_GH)
                                   + "): " + ", ".join(saved)
                                   + ". Streamlit передеплоит — маппинги подхватятся.")
                    else:
                        st.info("В GitHub не сохранено (проверьте токен/права репозитория).")
    ...
    # 8) СКАЧАТЬ аннотированный xlsx (цвет + комментарии по проблемам)
    st.divider()
    tag = {"Прогнать всё": "всё", "Только закуп": "закуп",
           "Только продажи": "продажи"}.get(res.get("mode"), "всё")
    st.caption("В файле проблемные ячейки подсвечены цветом и снабжены "
               "комментариями; лист «Легенда и проблемы» — расшифровка.")
    st.download_button(f"Скачать результат ({tag}, xlsx)",
                       data=report_xlsx.build(res),
                       file_name=f"SalesFlow_{tag}_{rdate:%Y_%m}.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    # №5: перенос остатков как отдельный CSV для следующего месяца.
    # Тот же файл в окне распознаётся как «carry» и подаётся как Остаток_нач.
    if results:
        cf = []
        for ch, out in results.items():
            m = out["movement"][["ID", "Остаток_конец"]].copy()
            m = m.rename(columns={"Остаток_конец": "Остаток_нач"})
            m.insert(0, "Канал", ch)
            cf.append(m)
        cf = pd.concat(cf, ignore_index=True)
        cf = cf[pd.to_numeric(cf["Остаток_нач"], errors="coerce").fillna(0) != 0]
        st.download_button(
            "Скачать перенос остатков (Остаток_нач для след. месяца)",
            data=cf.to_csv(index=False).encode("utf-8-sig"),
            file_name=f"carry_forward_{rdate:%Y_%m}.csv", mime="text/csv",
            help="В следующем месяце загрузите этот файл в то же окно — он "
                 "распознается как Остаток_нач и подхватится автоматически.")
